...
async function generateMsg(occasion, pointers) {
  ...
  try {
    const result = await generateMessage(contact, occasion, pointers);
    $('msgBubble').textContent = result.text;
    if (result.source === 'Template' && result.errors?.length) {
      const short = result.errors.map(e => e.replace(/\s*—\s*\{[\s\S]*$/, '').replace(/\s*—\s*\<[\s\S]*$/, '')).join('; ');
      $('msgSource').textContent = 'Template — AI unavailable (' + short + ')';
    } else {
      $('msgSource').textContent = result.source === 'Template' ? 'Template message' : `Generated by ${result.source}`;
    }
    $('msgLoading').hidden = true;
    ...
    $('msgSource').textContent = 'Template message';
  }
}
...
function openSettings() {
  ...
  $('debugInfo').textContent = `Cache: v17 | Provider: ${provider} | Groq: ${gqk ? gqk.substring(0, 8) + '...' : 'none'} | Gemini: ${gmk ? gmk.substring(0, 6) + '...' : 'none'}`;
